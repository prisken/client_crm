...
class WhatsAppService: ObservableObject {
    ...
    func sendMessage(
        ...
        let message = try await sendToBackend(payload: payload)
    ...
    private func sendToBackend(payload: WhatsAppMessagePayload) async throws -> WhatsAppMessage {
        ...
        let response = try JSONDecoder().decode(WhatsAppResponse.self, from: data)
        
        // Create WhatsApp message object
        let message = WhatsAppMessage()
        message.id = UUID(uuidString: response.messageId) ?? UUID()
        message.status = response.status
        message.sentAt = ISO8601DateFormatter().date(from: response.sentAt) ?? Date()
...
struct WhatsAppMessageView: View {
    ...
    private func sendMessage() {
        ...
        Task {
            do {
                let message = try await whatsappService.sendMessage(
                    to: client,
                    template: template,
                    variables: variables
                )
