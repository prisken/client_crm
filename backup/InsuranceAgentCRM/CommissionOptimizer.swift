...
class CommissionOptimizerService {
    ...
    func buildTaskList(
        ...
        let gap = (monthlyTarget - earnedSoFar) as NSDecimalNumber
        let targetAmount = monthlyTarget as NSDecimalNumber
    ...
    private func createOptimizedTask(
        ...
        let baseCommission = (task.estimatedCommission as NSDecimalNumber).doubleValue
    ...
    func calculatePerformanceMetrics(
        ...
        let totalCommission = tasks.reduce(0.0) { total, task in
            total + (task.estimatedCommission as NSDecimalNumber).doubleValue * task.probability
        }
...
class TodayTasksViewModel: ObservableObject {
    ...
    private func fetchMonthlyTarget() -> Decimal {
        ...
        do {
            ...
            return targets.first?.targetAmount ?? 0
        } catch {
